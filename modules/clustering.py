...
class Clustering(nn.Module):
    def __init__(self, *, device, num_clusters=10, d_model):
        ...
        self.cluster_k_proj = nn.Sequential(nn.Linear(d_model, 2*num_clusters, device=self.device),
                                            nn.GELU(),
                                            nn.Linear(2*num_clusters, num_clusters, device=self.device))

        self.cluster_q_proj = nn.Sequential(nn.Linear(d_model, 2*num_clusters, device=self.device),
                                            nn.GELU(),
                                            nn.Linear(2*num_clusters, num_clusters, device=self.device))
